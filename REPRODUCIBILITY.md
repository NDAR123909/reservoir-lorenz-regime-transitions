...
This repo is built to reproduce from a clean checkout. On the pinned environment a fresh run rebuilds the shipped figures bit for bit, not just statistically. This file records the seeds, the determinism guarantees, what is cached versus regenerated, and the one place a reader should expect tolerance rather than identity.

## Environment

Developed and validated on Python 3.12.3 with numpy 2.4.4, scipy 1.17.1, and matplotlib 3.10.8. The versions are pinned in `requirements.txt` and the interpreter is recorded in `.python-version`. The bit-for-bit guarantee is specific to this stack. The two version-sensitive pieces are the sparse-matrix RNG that builds the reservoir and the ARPACK eigensolver that reads its spectral radius, so a different numpy or scipy can produce a statistically equivalent run that is not bit-identical.
...
The pipeline is deterministic end to end on the pinned stack. The one spot that was not, and the fix:
...
This change is numerical plumbing. It does not alter any hyperparameter or the architecture; the target spectral radius is unchanged at 0.6.
